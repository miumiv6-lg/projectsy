...
import { Send, Bot, User, CreditCard, Bug, UserX, HelpCircle, Terminal, Sparkles, Plus, Copy } from 'lucide-react';
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'assistant',
      content:
        'Hello. I am the AI support assistant for Project SY. How can I help you today?'
    }
  ]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
        setMessages(prev => [...prev, {
            role: 'assistant',
            content: 'I understand. I can help you create a formal ticket for the administration regarding this issue.'
        }]);
        setIsLoading(false);
    }, 1000);
  };
      ...
      >
        ...
        {isLoading && (
           <div className="flex gap-3 animate-pulse">
              <div className="w-6 flex justify-center"><Sparkles size={16} className="text-zinc-600" /></div>
              <div className="text-zinc-500 text-xs mt-1">Generating...</div>
           </div>
        )}
      </div>
            ...
            <div className="mt-2 flex items-center justify-between px-1">
                <div className="flex items-center gap-3">
                   <div className="text-[10px] text-zinc-500 border border-zinc-800 px-1.5 py-0.5 rounded flex items-center gap-1">
                      Context <span className="text-zinc-700">None</span>
                   </div>
                   <div className="text-[10px] text-zinc-500 border border-zinc-800 px-1.5 py-0.5 rounded">
                      gpt-4o
                   </div>
                </div>
            </div>
